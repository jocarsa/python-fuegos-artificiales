...
for a in range(0, 1):
    ...
    for frame_num in range(total_frames):
        ...
        cv2.rectangle(frame, (0, 0), (width, height), (0, 0, 0), thickness=-1)
        ...
        # Convert to UMat for faster GPU operations
        frame_umat = cv2.UMat(frame)

        # Create a glow effect by blurring the frame using UMat (which can leverage the GPU)
        blurred_frame = cv2.GaussianBlur(frame_umat, (21, 21), 0)

        # Blend the original frame and the blurred frame using "add" mode for the glow effect
        glow_frame = cv2.add(frame_umat, blurred_frame)

        # Convert UMat back to a normal array for writing to the video
        final_frame = glow_frame.get()
