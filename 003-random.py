...
def generate_firework():
    ...
    speeds = np.random.uniform(2, 10, num_particles)  # Random speed for bursts
...
for frame_num in range(total_frames):
    ...
    # Generate a new firework every `firework_interval` frames
    if frame_num % 30 == 0:
        fireworks.append(generate_firework())

    # Update and draw each firework
    for fw in fireworks:
        x, y, dx, dy, colors, decay = fw
        ...
        # Draw particles as lines indicating speed and direction
        for i in range(len(x)):
            if 0 <= x[i] < width and 0 <= y[i] < height:
                start_point = (int(prev_x[i]), int(prev_y[i]))
                end_point = (int(x[i]), int(y[i]))
                cv2.line(frame, start_point, end_point, tuple(map(int, colors[i])), 2)
